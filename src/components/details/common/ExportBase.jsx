...
export async function exportGlobalTableXLS(
  ...
  const safeName = (advertiserInfo.name || "export")
    ...
    .replace(/_+/g, "_")
    .replace(/^_|_$/g, "");
  const filename = "Export_" + safeName + "_" + (advertiserInfo.id || "") + ".xlsx";
  ...
  const COLS = [
    ...
    { label: "Segment(s)", width: 28, align: "left"   },  // C7
    { label: "ListName",   width: 22, align: "left"   },  // C8
    { label: "Model(s)",   width: 20, align: "left"   },  // C9
    { label: "Agence",     width: 18, align: "left"   },  // C10
  ...
  for (const base of (bases || [])) {
    ...
    for (const brand of (base.brands || [])) {
      ...
      sc(row.getCell(5),  { fg: COLOR.black,    bg: rowBg, italic: true });

      row.getCell(6).value  = dates;
      sc(row.getCell(6),  { fg: COLOR.black,    bg: rowBg, align: "center" });

      row.getCell(7).value  = segments;
      sc(row.getCell(7),  { fg: COLOR.black,    bg: rowBg });
